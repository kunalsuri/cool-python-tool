...
- **Docstrings**: Use Google style for all functions/classes. Include `Args`, `Returns`, `Raises`.
...
- **Modern Idioms**: Use context managers (`with`), `enumerate`, `zip`, and structural pattern matching.

## 5. Anti-Patterns to Avoid
- Mutable default arguments (e.g., `def func(x=[])`).
- String concatenation in loops (use `.join()`).
- Global variables.
- Ignoring return values of functions that don't modify in-place.
